...
class ClusterCdkStack:
    ...
    def __init__(
        ...
        bucket: S3Bucket,
        compute_fleet_template_url: str,
        log_group_name=None,
        **kwargs,
    ) -> None:
        ...
        self._compute_fleet_template_url = compute_fleet_template_url
    ...
    def _add_fleet_and_scheduler_resources(self, cleanup_lambda, cleanup_lambda_role):
        ...
        if not self._condition_is_batch():
            self.compute_fleet_resources = CfnStack(
                self,
                template_url=self._compute_fleet_template_url,
                parameters={
                    CW_LOGS_CFN_PARAM_NAME: self.log_group,
                    "CleanupLambdaARN": cleanup_lambda.attr_arn,
                    "CleanupLambdaRole": cleanup_lambda_role.attr_arn,
                    "ComputeSecurityGroupID": self._compute_security_group.ref,
                    "HeadNodeIP": self._head_eni.attr_primary_private_ip_address,
                    "ClusterHostedZone": self.scheduler_resources.cluster_hosted_zone.ref
                    if self.scheduler_resources else None,
                    "ClusterDNSDomain": self.scheduler_resources.cluster_hosted_zone.name
                    if self.scheduler_resources else None,
                    "DynamoDBTable": self.scheduler_resources.dynamodb_table
                    if self.scheduler_resources else None,
                },
            )
...
# class ComputeFleetConstruct(CfnStack):
...
#     ):
#         super().__init__(scope, id, template_url=template_url, parameters=parameters)
