...
class ClusterCdkStack:
    ...
    def _add_resources(self):
        ...
        # Head node and Compute node security groups (Both managed and external)
...
class ComputeFleetConstruct(NestedStack):
    ...
    def _add_launch_templates(self, managed_placement_groups, instance_profiles):
        ...
        for queue in self._config.scheduling.queues:
            ...
            queue_stack = QueueNS(
                self,
                f"{queue.name}QueueStack",
                queue,
                managed_placement_groups,
                instance_profiles,
                self._compute_security_group,
                self._launch_template_builder,
                self._config,
                self._shared_storage_mount_dirs,
                self._shared_storage_attributes,
                self._shared_storage_infos,
                self._cluster_hosted_zone,
                self._dynamodb_table,
                self._log_group,
                self._head_eni,
            )
            compute_launch_templates[queue.name] = queue_stack.get_queue_launch_templates()
...
class QueueNS(NestedStack):
    def __init__(
        ...
        super().__init__(scope, id)
        self._launch_template_builder = launch_template_builder
        self._config = config
        ...
        self._log_group = log_group
        self._head_eni = head_eni
        self._cr_launch_templates = {}
        self._add_queue_resources(queue, managed_placement_groups, instance_profiles, compute_security_group)

    def get_queue_launch_templates(self):
        return self._cr_launch_templates

    @property
    def stack_name(self):
        return Stack.of(self.nested_stack_parent.nested_stack_parent).stack_name

    def _add_queue_resources(self, queue, managed_placement_groups, instance_profiles, compute_security_group):
        queue_lt_security_groups = get_queue_security_groups_full(compute_security_group, queue)
        queue_pre_install_action, queue_post_install_action = (None, None)
        if queue.custom_actions:
            queue_pre_install_action = queue.custom_actions.on_node_start
            queue_post_install_action = queue.custom_actions.on_node_configured
        ...
        for resource in queue.compute_resources:
            self._cr_launch_templates[resource.name] = self._add_cr_launch_template(
                queue,
                resource,
                queue_pre_install_action,
                queue_post_install_action,
                queue_lt_security_groups,
                ComputeFleetConstruct._get_placement_group_for_compute_resource(
                    queue, managed_placement_groups, resource
                ),
                instance_profiles,
            )

    def _add_cr_launch_template(
        self,
        queue,
        compute_resource,
        queue_pre_install_action,
