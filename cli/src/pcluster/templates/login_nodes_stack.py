...
from aws_cdk.core import CfnTag, Construct, Fn, NestedStack, Stack, CfnResourceSignal, CfnCreationPolicy, \
    CfnResourceAutoScalingCreationPolicy, CfnWaitCondition, CfnWaitConditionHandle
...
class Pool(Construct):
    ...
    def _add_resources(self):
        ...
        _, self.wait_condition_handle = self._add_wait_condition()
        self.wait_condition_handle_url = self.wait_condition_handle.ref
                    ...
                    Fn.sub(
                        get_user_data_content("../resources/login_node/user_data.sh"),
                        {
                            **{
                                ...
                                "WaitConditionURL": self.wait_condition_handle_url,
                            },
    ...
    def _add_wait_condition(self):
        wait_condition_handle = CfnWaitConditionHandle(
            self, id="LoginNodeWaitConditionHandle" + self.timestamp
        )
        wait_condition = CfnWaitCondition(
            self,
            id="LoginNodeWaitCondition" + self.timestamp,
